...
var Service = {
		...
		if (requestItems.length > 1) {
			...
			if (this._providers.get(serverName)) {
				...
				return new Promise(function (resolve, reject) {
					self.debug("service (1):" + service + " is offline, queue the task.");
	...
	registerState(state, provider) {
		this._states.set(provider.name, provider);
		if (!provider.name) {
			console.warn("Service.registerState: Provider should have a name.", provider);
		}
		this._statesByState.set(state, provider);
	},

	unregisterState(state, provider) {
