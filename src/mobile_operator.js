/**
 * seems to be a modified version of MobileOperator
 * @link https://github.com/mozilla-b2g/gaia/blob/975a35c0f5010df341e96d6c5ec60217f5347412/shared/js/mobile_operator.js
 */
const MobileOperator = {
	...
	userFacingInfo: function (mobileConnection) {
		var network = mobileConnection.voice.network,
			iccid = mobileConnection.iccId,
			iccObj = navigator.mozIccManager.getIccById(iccid),
			iccInfo = iccObj ? iccObj.iccInfo : null,
			operator = network ? network.shortName || network.longName : null;
		...
		if (this.isBrazil(mobileConnection)) {
			...
			var carriers = this.MOBILE_INFO.brazil.carriers;
			var regions = this.MOBILE_INFO.brazil.regions;

			carrier = carriers[network.mnc] || this.BRAZIL_MCC + network.mnc;
			region = regions[lac] ? regions[lac] + " " + lac : "";
		}

		return { operator: operator, carrier: carrier, region: region };
	},

	isBrazil: function mo_isBrazil(mobileConnection) {
		var cell = mobileConnection.voice.cell;
		var net = mobileConnection.voice.network;
		return net ? net.mcc === this.BRAZIL_MCC && cell && cell.gsmLocationAreaCode : null;
	},
